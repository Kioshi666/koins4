...
parameters = {
    ...
    "community-link":
        "http://aminoapps.com/c/anime-empire-1"
...
except:os.system("pip install pytz requests flask json_minify")
finally:
    import requests
    import random
    from pytz import timezone as tz
...
class App:
    ...
    def generation(self, email, password, device):
        ...
        try:
            ...
            for i2 in range(24): print(f"[\033[1;31mcoins-generator\033[0m][\033[1;35mmain-proccess\033[0m][{email}]: {self.client.send_active_object(comId = self.comId, timers = [{'start': int(time.time()), 'end': int(time.time()) + 300} for _ in range(50)], tz = self.tzc())['api:message']}."); time.sleep(3.99)
